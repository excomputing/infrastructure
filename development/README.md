...
> [!TIP]
> <a href="https://boto3.amazonaws.com/v1/documentation/api/latest/index.html" target="_target">Amazon Web Services Software Development Kit (Python)</a><br>
> Inspect `/usr/local/bin` for installation outlines of <abbr title="Amazon Web Services">AWS</abbr> <abbr title="Command Line Interface">CLI</abbr>, 'eksctl', and `kubectl`.<br>
...
```shell
docker build . --file .devcontainer/Dockerfile -t infrastructure
```
...
|:---------------|:-------|:---------|:---------|:---------|
| infrastructure | latest | $\ldots$ | $\ldots$ | $\ldots$ |


<br>

Subsequently, run an instance of the image `infrastructure` via:

```shell
docker run --rm -i -t -p 8050:8050 -w /app --mount
    type=bind,src="$(pwd)",target=/app 
    -v ~/.aws:/root/.aws infrastructure
```
...
The part `-v ~/.aws:/root/.aws` ascertains Amazon Web Services interactions via containers. Get the name of a running instance of ``infrastructure`` via:
